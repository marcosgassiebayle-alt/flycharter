...
export async function generateMetadata() {
  ...
  return { title: t("planesTitle"), description: t("planesDescription") };
}

async function getAircraft(): Promise<AircraftPreview[]> {
  const aircraft = await prisma.aircraft.findMany({
    where: { isActive: true, type: "PLANE" },
    include: {
      operator: {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-lg">{t("planesSubtitle")}</p>
      </div>
